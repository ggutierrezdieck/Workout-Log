...
//TODO: check why effect does not refresh when token changes
useEffect(()=>{
    if(token['mr-token'] !== "undefined" && token['mr-token'] != null)  window.location.href = '/workouts';
},[token])
...
const registerClicked = () => {
    ...
    .then(() => loginClicked()) //Calling login right after registration, it will use same login and password
    .catch(error => console.log(error))
}
